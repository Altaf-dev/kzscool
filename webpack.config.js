...
function generateHtmlPlugins(env, templateDir) {
  ...
  return templateFiles.map(item => {
    ...
    return new HtmlWebpackPlugin({
      ...
      inject: false,
      minify: false, // Отключение минификации HTML
    });
  });
}
...
const makeConfig = env => {
  return {
    ...
    mode: "production",
    optimization: {
      minimize: false, // Отключение минификации JS
      minimizer: [
        new TerserPlugin({
          extractComments: true,
          // Отключение минификации JS
          terserOptions: {
            compress: false,
          },
        })
          ...
          use: [
            {
              ...
              options: {
                publicPath: '../', // Если ваш CSS находится в подкаталоге
              }
            },
            {
              loader: "css-loader",
